...
//        TourismInformation doc = new TouristInformationMockBuilder().build();
        when(page.getModules()).thenReturn(Collections.emptyList());

//        when(linksFactory.getMegalinkModule(doc, Locale.UK)).thenReturn(new MultiImageLinksModule());
        builder.addModules(request);
//        Module module = ((List<Module>) request.getAttribute(PageTemplateBuilder.PAGE_ITEMS));
