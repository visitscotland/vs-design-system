...
import com.visitscotland.brmx.beans.Listicle;
...
class DocumentUtilsTest {
    ...
    @BeforeEach
    void init() {
        documentUtils = new DocumentUtils(utils);
    }
    ...
    private void mockPage(Page page, Node... documents) throws RepositoryException {
        ...
        when(page.getNode().getNodes().nextNode().getProperty(DocumentUtils.DOCUMENT_TYPE).getString()).thenReturn("visitscotland:Page");
    ...
    void getAllowedDocuments_notAllowedChildren() throws RepositoryException {
        ...
        when(listicleItemNode.getNodes().nextNode().getProperty(DocumentUtils.DOCUMENT_TYPE).getString()).thenReturn("visitscotland:ListicleItem");
    ...
    void getAllowedDocuments_nonParseableChildren() throws RepositoryException {
        ...
        Node listicleItemNode = mock(Node.class, withSettings().defaultAnswer(Mockito.RETURNS_DEEP_STUBS));
        when(listicleItemNode.getNodes().nextNode().getProperty(DocumentUtils.DOCUMENT_TYPE).getString()).thenReturn("hippo:document");
    ...
    void getAllowedDocuments_allowedChild() throws RepositoryException, QueryException, ObjectBeanManagerException {
        ...
        when(jcrNode.getNodes().nextNode().getProperty(DocumentUtils.DOCUMENT_TYPE).getString()).thenReturn("visitscotland:ListicleItem");
    ...
    void getSiblingDocuments_onlyAllowedChildren() throws RepositoryException, QueryException, ObjectBeanManagerException {
        ...
        when(dayNode.getNodes().nextNode().getProperty(DocumentUtils.DOCUMENT_TYPE).getString()).thenReturn("visitscotland:ItineraryDay");
