...
require("vue-styleguidist/scripts/utils/ensureWebpack")
  ...
  .then(function(mergedConfig) {
    ...
    switch (command) {
      case "build":
        ...
        break
      case "server":
        commandServer(mergedConfig)
        break
      default:
        commandHelp(mergedConfig)
    }
  })
  .catch(function(err) {
...
function commandBuild(config) {
  ...
  const build = require("vue-styleguidist/scripts/build")
...
function commandServer(config) {
  let spinner

  const server = require("vue-styleguidist/scripts/server")
