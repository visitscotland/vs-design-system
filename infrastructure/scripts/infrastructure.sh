...
findDynamicPorts() {
  ...
  for SERVICE in $VS_CONTAINER_SERVICE_LIST; do
    ...
    VS_CONTAINER_PORT_MAPPINGS="-p $"`eval VS_CONTAINER_EXT_PORT_$SERVICE`":$"`eval VS_CONTAINER_INT_PORT_$SERVICE`" $VS_CONTAINER_PORT_MAPPINGS"
...
sendBuildReport() {
  if [ -e "$VS_MAIL_NOTIFY_BUILD_MESSAGE" ] && [ "$VS_MAIL_NOTIFY_BUILD" == "TRUE" ]; then
    ...
    mailx -S smtp="$VS_MAIL_HOST" -s $VS_MAIL_NOTIFY_BUILD_SUBJECT -r $VS_MAIL_NOTIFY_BUILD_SENDER $VS_MAIL_NOTIFY_BUILD_TO < $VS_MAIL_NOTIFY_BUILD_MESSAGE
  fi
}
